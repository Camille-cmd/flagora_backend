...
class Command(BaseCommand):
    ...
    def fetch_prefectures_from_api(self):
        ...
        try:
            ...
            self.stdout.write(f"Successfully fetched {len(prefectures)} prefecture mappings")
            return prefectures

        except requests.exceptions.RequestException as e:
            self.stdout.write(self.style.ERROR(f"Failed to fetch prefectures data: {e}"))
            raise
    ...
    def fetch_departments_from_api(self):
        ...
        try:
            ...
            self.stdout.write(f"Successfully fetched {len(departments_data)} departments")
            return departments_data

        except requests.exceptions.RequestException as e:
            self.stdout.write(self.style.ERROR(f"Failed to fetch departments data: {e}"))
            raise
    ...
    def handle(self, *args, **options):
        try:
            ...
            # Combine the data
            combined_data = self.combine_department_data(departments_data, prefectures_data)

        except Exception as e:
            self.stdout.write(self.style.ERROR(f"Failed to fetch data from APIs: {e}. Aborting import."))
            return
